...
class Trainer:
    ...
    def create_optimizer_and_scheduler(self, num_training_steps: int, mode: int,alpha:float):
        """
        ...
        """
        self.create_optimizer(mode = mode)
    ...
    def create_optimizer(self,mode):
        """
        Setup the optimizer.
        ...
        if self.optimizer is None:
            ...
            """
                ...
                ##这里是冻结住的操作，只训练每一层的分类器
                train_params = []
                for n, p in opt_model.named_parameters():
                    if n.find('Policy_networks') != -1:
                ...
                optimizer_grouped_parameters = [
                    {
                        "params": [
                            p for n, p in train_params if (n in decay_parameters and p.requires_grad)
                        ],
                        "weight_decay": self.args.weight_decay,
                    },
                    {
                        "params": [
                            p for n, p in train_params if (n not in decay_parameters and p.requires_grad)
                        ],
                        "weight_decay": 0.0,
                    },
                ]
            elif mode == 3:
                ##这里是冻结住的操作，只训练每一层的分类器和lm_head

                ##两个方案：1.冻结backbone，全程只train classifiers and policy 2.lora train backbone + train classifiers and policy
                train_params = []
                for n, p in opt_model.named_parameters():
                    if n.find('lm_heads') != -1: #n.find('lm_heads') != -1:
                        train_params.append((n, p))
                        if p.requires_grad:
                            print('parameters: ', n)
                        else:
    ...
    def _inner_training_loop(
        self, batch_size=None, args=None, resume_from_checkpoint=None, trial=None, ignore_keys_for_eval=None, mode=None,alpha=None
    ):
        ...
        if not delay_optimizer_creation:
            self.create_optimizer_and_scheduler(num_training_steps=max_steps, mode=mode,alpha=alpha)
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                with self.accelerator.accumulate(model):
                    tr_loss_step = self.training_step(model, inputs, mode=mode,alpha=alpha)  ##这一步是关键

                    wandb.log({"train_loss":tr_loss_step})
    ...
    def training_step(self, model: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]], mode: int,alpha:float) -> torch.Tensor:
        """
        ...
        #print('inputs: ',inputs)  ##需要看一下这里inputs的batchsize是多大，
        ...
        if mode == 2 or mode == 3:
            ...
            if sample_K >= 1:
                ...
                for key, value in inputs.items():
                    ...
                    new_batch[key] = tmp
                inputs = new_batch  # 这里把新*8后的batch放入到inputs当中

            #print('inputs: ',inputs)

        #print('new_inputs: ', inputs)
        ...
        with self.compute_loss_context_manager():
            loss = self.compute_loss(model, inputs,mode=mode,alpha=alpha)
            #print('loss1: ',loss)
    ...
    def compute_loss(self, model, inputs, return_outputs=False, mode=1,alpha=0.0001):
        """
        ...
        #print('inputs: ',inputs)
        outputs = model(**inputs,mode=mode,alpha=alpha)
    ...
    def evaluate(
        ...
        print('self.args.use_legacy_prediction_loop: ',self.args.use_legacy_prediction_loop)
    ...
    def evaluation_loop(
        ...
        for step, inputs in enumerate(dataloader):
            ...
            # Prediction step
            loss, logits, labels = self.prediction_step(model, inputs, prediction_loss_only, ignore_keys=ignore_keys, mode=mode,alpha=alpha)
    ...
    def prediction_step(
        ...
        with torch.no_grad():
            if is_sagemaker_mp_enabled():
                raw_outputs = smp_forward_only(model, inputs) #目前的mode也还没有传入到这个输入里面去，因为还不清楚 generate这个函数的过程
                if has_labels or loss_without_labels:
